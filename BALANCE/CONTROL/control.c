...
**************************************************************************/
///唐总自定义变量
float V_sum = 15;   //公转线速度，单位：脉冲/s
u8 status =1;
///余乔自定义变量
float vehicle_radius = 10;    //小车外接圆半径，单位毫米
float turn_radius = 12;     //公转半径，单位毫米
float wheel_radius = 127/2;     //车轮半径，单位毫米
	  ...
	  if(mode==0)//速度
		{	
				 ...
				 case 2:  
					 ...
					 //Move_Y+=step;  
					 //Flag_Move=1; 
					 if(status==1&&angle<=270&&angle>10)//过滤angle=0/360的情况
					 {
						 Flag_Direction=1;
						 status=2;
					 }
					 else if(status == 2&&angle<=180){
						 Flag_Direction=3;
						 status=3;
					 }
					 else if (status==3 && angle<=90){
						 Flag_Direction=5;
						 status=4;
					 }
					 //--------------------------------以上status系列不用管，重点是下面
					 ...
					 {
					 Move_X=V_sum*cos((angle+180)*PI/180);  	//==  V_sum * cos(angle)    X轴方向线速度
				   Move_Y=V_sum*sin((angle+180)*PI/180);   	//== -V_sum * sin(angle)    Y轴方向线速度
				   Move_Z=V_sum/1.2;												//自转和公转角速度一致，所以线速度 = V_sum* 小车半径 / 公转半径
					 }
					 //相对坐标系转绝对坐标系
